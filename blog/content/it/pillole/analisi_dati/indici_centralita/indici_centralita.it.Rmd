---
title: "Indici di centralità: quali sono e come possono trarre in inganno"
date: 2021-06-08
modified: 
description: "Gli indici di centralità sono alla base della statistica descrittiva. Vediamo quali sono e quali sono i loro limiti di utilizzo"
draft: false
hideToc: true
enableToc: false
...
```{solution}
<br>
Per calcolare le medie aritmetiche di ogni ospedale, sommo i valori di glucosio dei pazienti di quell’ospedale e divido il valore ottenuto per il numero dei soggetti. 
...
Nel pannello A, si vede come la riga rossa corrispondente alla mediana abbia sia alla sua sinistra, sia alla sua destra 5 punti, corrispondenti ai valori di glucosio trovati nei pazienti dell’ospedale 1.
Nel pannello B, invece, la linea rossa corrisponde ad un valore del campione (come dovrebbe essere visto che il campione ha numerosità dispari). Sia a sinistra, sia a destra dela mediana ci sono 7 punti. <br>
Questo dimostra visivamente l’effettiva divisione a metà che la mediana attua sul campione.

## Moda
Per ultimo, vi parlo brevemente dell’indice di centralità chiamato “moda”. Ho detto brevemente, perché non c’è molto da dire su questo indice, in quanto è molto poco utilizzato e non è propriamente un indice di centralità (nella sezione dei limiti vi spiego il perché). 
La moda di un campione è quel valore che ha più occorrenze, ovvero che “compare” di più. Ne consegue che è un valore <i>interno</i> al campione, sempre.

```{example, name="Moda", label="md"}
Sempre prendendo spunto dai dati sugli ospedali dell’esempio \@ref(exm:mar), calcolare la moda.
```
<br>
```{solution}
<br>
Nel primo ospedale, ci sono due pazienti con valore 94 e due pazienti col valore 102, tutti gli altri hanno valori diversi tra loro. Ne consegue che la moda in realtà sia data da due valori, e che la distribuzione sia definita <i>bimodale</i>. <br>
Nel secondo ospedale, invece, tutti i pazienti hanno valori diversi. In questo caso si potrebbe ricorrere alla distribuzione di frequenza e valutare quale sia l’intervallo più rappresentato (ma questo argomento non verrà trattato in questo post).
```
## Recap grafico
Di seguito si vede un recap grafico degli indici di centralità dei campioni presi in esempio:
```{r all, echo=FALSE, fig.align='center', fig.cap="Rappresentazione grafica dei campioni e dei loro indici di centralità", fig.width=10}
valori1 <- data.frame(
...
Nel pannello A vediamo gli indici relativi all’ospedale 1, in cui abbiamo i seguenti valori:
<li>Media: 97.7</li>
<li>Mediana: 98</li>
<li>Moda: 94 e 102</li>
Come si può vedere, media e mediana non hanno valori molto diversi tra loro, mentre la moda non solo ha 2 valori, ma questi valori sono anche distanti da media e mediana.

Nel pannello B invece, gli indici relativi all’ospedale 2:
<li>Media: 102.13</li>
<li>Mediana: 102</li>
<li>Moda: NA</li>
...
## Limiti
Eccoci arrivati alla parte più importante di questo post, che cercherà di farvi capire la debolezza di questi indici quando usati <b>da soli</b> per descrivere un campione.  

Partiamo dai dati che abbiamo visto nel recap. Balza subito all’occhio come la moda in sé possa dire solo se un campione sia unimodale o multimodale, ma per il confronto tra vari campioni c’è bisogno di avere valori più indicativi e precisi.

Passiamo ora alla media: il più grande problema risiede nella sua suscettibilità a valori estremamente grandi o estremamente piccoli.

```{example, name="suscettibilità media", label = "extreme"}
Mettiamo il caso che nel primo ospedale, al posto di avere un paziente con 105 ne avessimo uno diabetico con 150, la media a quel punto passerebbe da 97.7 a 102.2! Il che porterebbe il valore medio dell’ospedale 1 ad essere più alto rispetto a quello dell’ospedale 2, e solo per il fatto che un paziente è diabetico.
```

I valori estremi hanno una grande influenza sulla media, ma <u>non</u> sulla mediana: infatti, anche usando i dati dell’esempio \@ref(exm:extreme),  la mediana dell’ospedale 1 rimarrebbe 98, in quanto contano solo i 2 valori centrali, che in questo caso non sono cambiati.<br>
Questo però porta ad un’altra considerazione sui limiti di questi indici: 98 e 102.2 sono molto diversi come valori. In pratica, ci sono casi in cui media e mediana hanno valori molto diversi tra loro, e l’utilizzo di uno piuttosto che dell’altro potrebbe avere effetti diversi nella percezione di chi legge.

Per esempio, usando i dati modificati si potrebbe dire: “Confrontando le medie dei valori di glucosio nei due ospedali, non sembra esserci differenza tra i pazienti” oppure “Confrontando le mediane dei valori di glucosio dei pazienti dei due ospedali in questione, sembrerebbe che i pazienti dell’ospedale 1 abbiano dei valori inferiori di glucosio pari a circa 4 mg/dl”. Stessi dati, conclusioni diverse.  
...
Tornando ai dati dell’esempio \@ref(exm:mar). Trasformiamo ora “Ospedale 1” con “farmaco X” e “Ospedale 2” con “placebo” e mettiamo il caso di essere in uno studio scientifico volto a valutare l’efficacia del farmaco X nel ridurre i valori di glucosio nel sangue (ovviamente gli studi si fanno con numeri molto maggiori, questo è solo un esempio). <br>
Avremmo dunque: media placebo 102.13 e media farmaco X 98.<br>
A questo punto, qualcuno potrebbe far uscire una notizia dal titolo: il farmaco X riduce i valori di glucosio nel sangue di circa 4 mg/dl. Wow, questo può aiutare nella lotta contro l’iperglicemia!<br>
E invece no! Questa notizia non uscirebbe mai, dal momento che chi conduce lo studio (giustamente) non si può fermare a confrontare solo le medie ed è “costretto” a fare un test d’ipotesi di confronto tra i due campioni, che fornirebbe le seguenti conclusioni: non c’è una forza statistica tale da rifiutare il fatto che i due campioni in realtà siano uguali (p-value 0.1142). Dunque, il farmaco non ha un effetto nella riduzione dei valori di glucosio nel sangue.

Non preoccupatevi se questa parte vi risulta ostica e di difficile comprensione, affronterò il tema dei test d’ipotesi per il confronto di due campioni più avanti, quando avrò introdotto anche altri concetti utili. Concetti che sono alla base della statistica, ovvero gli indici di dispersione, che ci spiegano <i>come</i> sono sparsi i nostri dati attorno alla media/mediana. 

## Conclusioni
Gli indici di centralità sono alla base della statistica descrittiva di un campione, ci permettono di evidenziare quali sono dei valori importanti del campione, attorno ai quali si distribuiscono tutti gli altri. <b>MA</b>, questi indici da solo <u>non</u> bastano, e non devono essere utilizzati in maniera fuorviante.
